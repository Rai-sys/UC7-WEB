...
class CursoController {
	...
	static async criar(req, res) {
		try {
			...
			if (cod_curso && nome) {
				...
				res.status(201).json({ msg: 'Curso cadastrado com sucesso!', curso });
			} else {
	...
	static async listar(req, res) {
		try {
			...
			res.status(200).json({ msg: 'Nenhum curso encontrado.' })
		} catch (error) {
			res.status(500).json({ erro: error.message })
		}
	}
	...
	static async atualizar(req, res) {
		try {
			...
			if (nome && cod_curso) {
				...
				res.status(200).json({ msg: 'Curso atualizado com sucesso!', curso });
			} else {
				res.status(400).json({ msg: 'Todos os campos devem ser preenchidos.' })
			}
		} catch (error) {
